...
DUMMY_PASSWORD_HASH = "$pbkdf2-sha256$600000$Zex97x3jPKd0bm0tZSwFoA$klp5Ce9bGtgIrav1P4n2/7yGM9gmX5SFof3at1dSTYw"
...
def get_session(request: Request, s: Session):
    ...
    if not st or ensure_aware(st.expires_at) < now():
        return None
    return st
...
def verify_email(token: str, request: Request, response: Response, s: Session = Depends(db)):
    ...
    if not row or ensure_aware(row.expires_at) < now():
