...
def test_v110_contract():
    ...
    assert (
        "/api/v1/patterns" in main
        and "/api/v1/insights" in main
        and "/api/v1/signals" in main
        and "/api/v1/profile" in main
        and "/api/v1/reports/weekly" in main
    )
    ...
    assert "GOOGLE_CLIENT_ID" in main
    assert "AI_ANALYSIS_ENABLED" in main
    assert (
        ("v1.1.0" in readme or "v1.0.0" in readme)
        and "NexGene Signals" in readme
        and "Weekly NexGene Report" in readme
    )
